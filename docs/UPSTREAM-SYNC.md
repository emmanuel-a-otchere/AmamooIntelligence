...
OtchereJarvis tracks [`open-jarvis/OpenJarvis`](https://github.com/open-jarvis/OpenJarvis). This document is the playbook for keeping the fork in sync without breaking downstream specializations.
...
# 4. Resolve any conflicts (rare — OtchereJarvis adds few files on top of upstream)
...
- Upstream patches to files OtchereJarvis does **not** modify (i.e., anything under `src/openjarvis/core/`, `desktop/`, `frontend/`, etc., outside the use-cases scaffold). These cannot conflict.
...
- Upstream changes to `docs/` — accept, then re-apply any OtchereJarvis-specific docs (`SPECIALIZING.md`, `UPSTREAM-SYNC.md`).
...
Downstream forks pull from OtchereJarvis `main`, not from upstream, so this sync is the contract.

## Conflict resolution principles

If upstream changes a file OtchereJarvis also touched (currently only `docs/SPECIALIZING.md`, `docs/UPSTREAM-SYNC.md`, and `use-cases/`):

1. Keep OtchereJarvis-specific intent (this fork exists to be a specialization base).
